---
description: "The episodic ledger's Ed25519 + SHA3-256 sigchain: 18-field AuditEvent schema, hash chaining, tamper detection, Hybrid Logical Clock timestamps, and kernel-reserved event types."
---
...
The kernel writes the following event types to the sigchain.
Application code MUST NOT use these prefixes:
...
Complication outcome events use the `action.outcome.*` prefix
(Section 11.6 of the specification). Application events should
use a namespaced prefix: `publisher.category.name`
