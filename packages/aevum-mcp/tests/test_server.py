...
def test_commit_tool_returns_envelope() -> None:
    ...
    tool_fn = _get_tool_fn(mcp, "commit")
    result = tool_fn(
        event_type="app.test",
        payload={"k": "v"},
        actor="mcp-user",
...
def test_ingest_tool_requires_consent() -> None:
    ...
    tool_fn = _get_tool_fn(mcp, "ingest")
    result = tool_fn(
        data={"content": "test"},
        provenance=_prov(),
        purpose="mcp-testing",
